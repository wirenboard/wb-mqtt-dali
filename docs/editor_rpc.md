    ...
    "buses": [
      {
        ...
        // Группы, существующие на шине: те, в которые входит хотя бы один балласт, и те,
        // для которых в конфиге задан блок on_off — такая попадает в список и без участников.
        "groups": [
          {
            // UID группы — его принимают Editor/GetGroup и Editor/SetGroup
            "id": "wb-mdali_1_bus_1_g3",

            // Номер DALI-группы, 0..15
            "number": 3
          }
        ],
...
Прочитать текущие групповые настройки и их схему.
...
```jsonc
{
  ...
  "groupId": "wb-mdali_1_bus_1_g0"
}
```

### Ответ

Та же форма, что у `Editor/GetDevice`:
...
```jsonc
{
  ...
  // JSON Schema групповых параметров, слитая по участникам группы.
  "schema": { ... }
}
```
...
Та же форма, что у `Editor/GetGroup`, — настройки группы и их схема после записи:
...
Пустые `config` и `schema` означают, что группы больше нет.
