...
- **Docstrings on non-trivial tests** — start the test with a docstring of at most two lines: what's set up and what's expected, not a retelling of the body. Trivial one-liners (single assertion against a pure function) don't need it; anything with multi-step setup, async interactions, or non-obvious expectations does.
- **Comments: hard caps, no rationale** — a comment states an invariant or an external contract a future edit would break; nothing else. Why this design was chosen, which alternatives were rejected, how the old code behaved: plan doc and commit message, never code. Caps: class or module docstring ≤ 3 lines of text, and a mechanism is explained once in the whole codebase; method docstring ≤ 1 line or none, never repeating the class docstring; inline comment ≤ 1 line; test docstring ≤ 2 lines. Anything over a cap is cut, not rewrapped.
