...
class ActualLevelControl(MqttControlBase):
    def __init__(self, dimming_curve_state: DimmingCurveState) -> None:
        super().__init__(
            ControlInfo("actual_level", ControlMeta(title=TranslatedTitle("Actual Level", "Фактический уровень"), read_only=True, units="%"), "0")
        )
...
CONTROLS: list[MqttControlBase] = [
    MqttControl(
        ControlInfo("error_status", ControlMeta("alarm", TranslatedTitle("Error Status", "Статус ошибки"), read_only=True), "0"),
    ...
    MqttControl(
        ControlInfo("recall_max_level", ControlMeta("pushbutton", TranslatedTitle("Recall Max Level", "Максимальный уровень"))),
        commands_builder=lambda short_address, _: [RecallMaxLevel(GearShort(short_address))],
    ),
    MqttControl(
        ControlInfo("recall_min_level", ControlMeta("pushbutton", TranslatedTitle("Recall Min Level", "Минимальный уровень"))),
        commands_builder=lambda short_address, _: [RecallMinLevel(GearShort(short_address))],
    ),
    MqttControl(
        ControlInfo("step_down_and_off", ControlMeta("pushbutton", TranslatedTitle("Step Down And Off", "Шаг вниз и выкл"))),
        commands_builder=lambda short_address, _: [StepDownAndOff(GearShort(short_address))],
    ),
    MqttControl(
        ControlInfo("on_and_step_up", ControlMeta("pushbutton", TranslatedTitle("On And Step Up", "Вкл и шаг вверх"))),
        commands_builder=lambda short_address, _: [OnAndStepUp(GearShort(short_address))],
    ),
    MqttControl(
        ControlInfo("dapc", ControlMeta("text", TranslatedTitle("Direct Arc Power Control", "Задать мощность (DAPC)")), ""),
