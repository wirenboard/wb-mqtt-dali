...
class HoldTimerParam(InstanceParam):
    def __init__(self, instance_number: InstanceNumber) -> None:
        ...
        self.description = TranslatedTitle(
            ...
            ru=(
                "Применимо только к датчикам движения, но не датчикам присутствия. После последнего обнаруженного движения "
                "состояние «Занято» удерживается в течение этого времени, и каждое новое движение "
                "запускает отсчёт заново. Когда время без движения истекает, состояние сменяется на "
                "«Вакантно»."
            ),
        )
